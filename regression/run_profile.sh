...
while [ "$i" -lt "$SPLITS" ]; do
    ...
    "${PETASEARCH}" createkmertable "$RESULTS/target_sra_${i}_${SPLITS}" "$RESULTS/target_kmer_${i}_${SPLITS}" --spaced-kmer-mode 0 -k 6 #--seed-sub-mat PAM30.out
...
"${PETASEARCH}" petasearch "${QUERYDB}" "$RESULTS/targetdbs" "$RESULTS/resultlist" "$RESULTS/results_aln.m8" "$RESULTS/tmp" \
    --comp-bias-corr 1 -e inf --mask 1 --k-score 0 --max-kmer-per-pos 20 -k 6 \
    --spaced-kmer-mode 0 --exact-kmer-matching 0 --req-kmer-matches 1 --xdrop 10000 \
    --format-output query,target,fident,alnlen,mismatch,gapopen,qstart,qend,tstart,tend,evalue,bits #,cigar,qaln,taln
